...
| `phi` | Def. 3.1 (fourth component replaced 2026-09-08) | mean of the available components (`n_components`): four 250-day robust z-scores (funding, OI/cap, −VRP, −stablecoin growth) and the bounded range-position score 4·(pos₉₀ − ½) | `fragility` | `state.fragility_index` | `test_fragility_index_mean_of_available_components` |
...
### Resistance / support tests (work order 8, panel 9)

| indicator | ref | definition | source | code | test |
|---|---|---|---|---|---|
| resistance / support test | §1 | up-move (5d & 3d return > 0) into a level, close within 2% of R, approach from below over 3 days; mirror for support; deduped to one event per resolution window | `resistance_model`, daily.json `resistance.active` | `compute.resistance.build_events` | `test_events_carry_three_definitions_and_both_sides` |
| reference level R (×3) | §1 | 90-day high, most recent confirmed swing high (5-bar fractal), nearest high-volume node above price (90-day volume-by-price); all causal as of t−1, all reported | `resistance_model` | `compute.resistance._rolling_max_shift`/`_swing_levels`/`_vp_levels` | `test_reference_levels_are_causal_no_lookahead` |
| break / reject / chop label | §1 | break = close beyond R by 0.5% held 2 closes; reject = close back through R with a lower low/higher high than the pre-test swing within the horizon; else chop | `resistance_model` | `compute.resistance._label_side` | `test_break_and_reject_labels` |
| P(direction) — base rate | §3a/§7a | walk-forward multinomial logit on price/structure + market context, full history from 2018-06-21; week-clustered s.e., out-of-sample calibration; publishes above the 20-event floor | daily.json `resistance.cells[].base_rate` | `compute.resistance.fit_direction` | `test_base_rate_publishes_and_size_asymmetry` |
| E(move \| direction) — magnitude | §3b | mean continuation (break) or maximum adverse excursion (reject) at 5/20d, Newey–West s.e.; the size asymmetry that makes P×E meaningful | daily.json `resistance.cells[].magnitude` | `compute.resistance.magnitude_summary` | `test_base_rate_publishes_and_size_asymmetry` |
