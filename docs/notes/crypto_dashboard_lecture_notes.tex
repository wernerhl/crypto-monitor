...
\subsection{Corrections to the notes (dated)}\label{sec:impl-corrections}

These correct claims made in the body of the notes; the body is left as written so that the correction and its exhibit stay visible.

\paragraph{2026-09-08, on \S\ref{sec:state} (variance risk premium).} The body says that a collapse of the VRP to zero or below historically precedes volatility expansions. On the 2021--2026 BTC sample (84 flags in 10 episodes, exhibit: methods page, ``Rule 4.3 by driver''), a negative VRP is a transient state after which realised vol over the next 30 days fell \emph{below} the implied vol at the flag in about four flags of five, across drivers (complacency 21 percent, post-shock 18 percent, neither 50 percent on six elapsed flags). The expansion pattern appears only when implied vol is low and realised vol is already rising (the ``both'' cell: 6 of 7 flags, 3 episodes), on too few episodes to state. Review decision 2 (2026-09-08) removes the Rule 4.3 action (the vol-selling prohibition); the reading and its driver stay on the market-state panel; no replacement rule.

\paragraph{2026-09-08, on \S\ref{sec:state} and Definition~\ref{def:fragility} (the fragility index).} The equal-weighted five-component $\Phi$ has not been validated as a predictor of post-shock damage: on 147 BTC shock days (2018-06 to 2026-06; exhibit: \texttt{docs/notes/fragility\_validation.md}) the 20-day drawdown, the next-20-day realised vol and the time to recover do not increase in $\Phi_{t-1}$, and the 5-day drawdown moves the other way at $|t| = 2.3$. Two qualifications: the leverage components have months of walk-forward history (the funding $z$ exists from July 2026 and the open-interest-over-cap $z$ from April 2026, so they were available on 0 and 3 of the 147 shock days), so they are untested rather than refuted; and the range-position component reads as a bull-regime marker in this sample (at the top of the 90-day range in a rising market, shocks were shallower and recovered faster). Review decision 3 (2026-09-08) keeps $\Phi$ as a positioning summary with descriptive state words (deleveraged / neutral / levered) and withdraws the predictive claim pending leverage-component history from an aggregated source.

\paragraph{2026-09-08, on \S\ref{sec:supply} and Rule 4 (unlock cliffs).} The placebo result (pseudo-cliff dates of the same tokens read the same as real cliffs; \texttt{docs/notes/pre\_unlock\_drift.md}) and review decision 1 reclassify the cliff rule as a calendar; see the previous subsection.

\paragraph{2026-09-08, on Section~\ref{sec:governance} (review procedure).} Removing an unsupported claim on full-history evidence is within the review procedure and does not wait for the quarterly calendar; the decision, its exhibit and its date are recorded in the changelog and here. Fitting a parameter does wait for the calendar, and needs the written case that the mechanism changed.
